...
| the gen-fc family | the C++ app scaffold (lib + main + impl) — the single C++-from-`.art` path |
...
`gen-proto-package` (gen-fc emits per-package `.proto` internally),
`gen-cpp-stubs` (conflicted with the gen-fc family, which emits the
