...
int main() {
    ...
    IncrementerNode incrementer_node;
    incrementer_node.start();
    std::fprintf(stderr, "[incrementer_node] up — TIPC type=0x%x instance=%u\n",
                 IncrementerNode::kTipcType, IncrementerNode::kTipcInstance);

    if (auto* incrementer_node_cfg = config_mux.bind_node(
            incrementer_node, IncrementerNode::kTipcType,
            IncrementerNode::kTipcInstance)) {
        config_mux.register_cast<platform_runtime_LogLevelPush>(
            incrementer_node_cfg, incrementer_node);
        ...
        std::fprintf(stderr,
                     "[incrementer_node] WARN: config service bind failed; "
