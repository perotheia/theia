...
int main(int argc, char** argv) {
    ...
    // RUNTIME NODE IDENTITY = the PROTOTYPE name ("observer") — the name the
    ...
    // LOCAL node this equals ObserverNode::kNodeName; for an IMPORTED package node it
    ...
    // Per-node logger: tagged [#observer] (matches `tdb ps`), sink chosen by
    ...
    {
        auto observer_log = MakeContextLogger("observer");
    ...
    ::theia::runtime::resolve_node_tipc("observer",
    ...
    observer.set_tipc_instance(observer_inst);
    ...
    const auto observer_params =
        ::theia::runtime::get_config().node("observer");
    ...
    const bool observer_enabled = observer_params.boolean("enabled", true);
    if (!observer_enabled) {
        observer.log().info("node disabled (params.enabled=false) — not "
    ...
    if (auto* observer_cfg = config_mux.bind_node(
        ...
        observer.pg_attach("observer", observer_cfg,
                                observer_type, observer_inst);
    } else {
    ...
    }  // end if (observer_enabled) — PASS 1 (mux wiring)
    ...
    if (observer_params.boolean("enabled", true)) {
    if (auto observer_delay = observer_params.u32("start_delay_ms", 0))
        std::this_thread::sleep_for(std::chrono::milliseconds(observer_delay));
    observer.start();
    {
        char _tipc[64];
    ...
    ::theia::runtime::apply_node_affinity(observer.native_handle(),
        "observer", std::getenv("THEIA_NODE_CFG"));
    ...
    // node, own timer thread; 1s default matches the supervisor's check cadence.
    {
        auto observer_hb = std::make_unique<
            ::theia::runtime::HeartbeatPublisher>("observer");
        if (observer_hb->open()) {
    ...
    }  // end if (observer_enabled) — PASS 2 (start + heartbeat)
