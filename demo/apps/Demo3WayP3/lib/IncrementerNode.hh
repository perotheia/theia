...
inline void IncrementerNode::broadcast_inc_out_inc(const Inc& msg) {
    ...
    // `msg` to EACH watched member. group identity = msg_type_name<Inc>().
    // The impl can call pg_members<Inc>() first to skip work when the list
    // is empty (e.g. logcat stop-tailing) — this default just fans out to all.
    pg_watch<Inc>();
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
    if (!pb_encode(&_os, ::theia::runtime::RemoteCodec<Inc>::fields(), &msg))
        return;
    pg_.broadcast_members<Inc>(_buf, static_cast<uint16_t>(_os.bytes_written));
}
