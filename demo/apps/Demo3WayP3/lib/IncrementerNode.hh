...
// impl/IncrementerNode_handlers.cc.
...
#include "impl/IncrementerNode_state.hh"
...
class IncrementerNode : public ::theia::runtime::GenServer<IncrementerNode, IncrementerNodeState> {
public:
    static constexpr const char* kNodeName = "incrementer";
    static constexpr uint32_t kTipcType     = 0xd0010005u;
    ...
    using ::theia::runtime::GenServer<IncrementerNode, IncrementerNodeState>::handle_cast;
    ...
    // (post_info / self-tick).
    void terminate(const char* /*reason*/, IncrementerNodeState& /*s*/) noexcept {}
    ...
    // ---- OTP init/1 — body in impl (IncrementerNode_handlers.cc) ----------
    //
    ...
    void init(IncrementerNodeState& s);
    ...
    void handle_info(const char* info, IncrementerNodeState& s);

    // ---- config update — body in impl (IncrementerNode_handlers.cc) -------
    //
    // This node binds `config IncrementerConfig`. services/per casts a
    ...
    // ParseFromString cfg.config into IncrementerConfig and honor the changed mask.
...
inline uint32_t IncrementerNode::subscribe_inc_out_inc(
        Inc_outIncSubscriber s) {
...
inline void IncrementerNode::unsubscribe_inc_out_inc(uint32_t id) {
    std::lock_guard<std::mutex> lk(inc_out_inc_mu_);
...
inline void IncrementerNode::broadcast_inc_out_inc(const Inc& msg) {
