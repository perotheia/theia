...
4. ~~add command completion~~ — done. `theia.py` at workspace
   root is a click-based workspace launcher (mosaic.py-style)
   wrapping artheia + bazel + repo + supervisor bringup; click's
   built-in completion works for both `artheia` and `theia`. See
   `docs/artheia/completion.md` for the shell setup snippets.