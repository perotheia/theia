...
// source: system/packages/v2v/package.art
//
...
#include "v2v_codecs.hh"   // FC-wide inbound RemoteCodecs (#387)
#include "packages/v2v/v2v.pb.h"
...
// ara::v2v`) so it isn't nested under this file's
// namespace block — bound as the GenServer State type below.
#include "impl/OsiV2v_state.hh"
...
// (packages_v2v_*); aliasing them here keeps callers'
// signatures readable.
using Beacon = packages_v2v_Beacon;
using ConstellationUpdate = packages_v2v_ConstellationUpdate;
using GetConstellationReq = packages_v2v_GetConstellationReq;
using GetAlertDecisionReq = packages_v2v_GetAlertDecisionReq;
using AlertDecision = packages_v2v_AlertDecision;
using InjectBeaconReq = packages_v2v_InjectBeaconReq;
using InjectBeaconReply = packages_v2v_InjectBeaconReply;
...
class OsiV2v : public ::theia::runtime::GenServer<OsiV2v, OsiV2vState> {
public:
    static constexpr const char* kNodeName = "osi_v2v";
    static constexpr uint32_t kTipcType     = 0x800100A0u;
    ...
    using ::theia::runtime::GenServer<OsiV2v, OsiV2vState>::handle_cast;
    ...
    // (post_info / self-tick).
    void terminate(const char* /*reason*/, OsiV2vState& /*s*/) noexcept {}
    ...
    void handle_cast(const Beacon& msg, OsiV2vState& s);
    ...
    // ---- OTP init/1 — body in impl (OsiV2v_handlers.cc) ----------
    //
    ...
    void handle_info(const char* info, OsiV2vState& s);

    // ---- config update — body in impl (OsiV2v_handlers.cc) -------
    //
    // This node binds `config V2vConfig`. services/per casts a
    ...
    // ParseFromString cfg.config into V2vConfig and honor the changed mask.
    ...
    void broadcast_broadcast_upd(const ConstellationUpdate& msg);
...
inline void OsiV2v::broadcast_broadcast_upd(const ConstellationUpdate& msg) {
    // Ensure we're monitoring the group (OTP pg:monitor — idempotent), then cast
    // `msg` to EACH watched member. group identity = msg_type_name<ConstellationUpdate>().
    // The impl can call pg_members<ConstellationUpdate>() first to skip work when the list
    // is empty (e.g. logcat stop-tailing) — this default just fans out to all.
    pg_watch<ConstellationUpdate>();
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
    if (!pb_encode(&_os, ::theia::runtime::RemoteCodec<ConstellationUpdate>::fields(), &msg))
        return;
    pg_.broadcast_members<ConstellationUpdate>(_buf, static_cast<uint16_t>(_os.bytes_written));
}
