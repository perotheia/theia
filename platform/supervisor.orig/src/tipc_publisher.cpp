// TIPC publisher/receiver implementation. See header for design notes.
...
bool TipcPublisher::reply_to(int client_fd, uint16_t type_tag,
    ...
    ssize_t n = ::send(client_fd, buf.data(), buf.size(), MSG_NOSIGNAL);
    if (n < 0) {
...
void TipcPublisher::poll() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Inbound frame. SEQPACKET preserves boundaries — one recv()
        // returns exactly one application frame (with the 2-byte tag).
        char buf[65536];
        ...
        if (r < 2) continue;  // malformed; need at least the tag

        uint16_t tag = (static_cast<uint8_t>(buf[0]) << 8) |
                       static_cast<uint8_t>(buf[1]);
        if (inbound_cb_) {
            inbound_cb_(fd, tag,
                        std::string(buf + 2, static_cast<size_t>(r) - 2));
        }
    }
}
