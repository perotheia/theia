...
// source: platform/supervisor/system/component.art
//
...
inline void SupervisorCtl::broadcast_events_event(const SupervisionEvent& msg) {
    std::vector<EventsEventEntry> snap;
    {
        std::lock_guard<std::mutex> lk(events_event_mu_);
        snap = events_event_subs_;
    }
    for (const auto& e : snap) {
        try { e.fn(msg); }
        catch (...) {
            std::fprintf(stderr,
                "[%s] subscriber %u threw on events.event — dropping\n",
                kNodeName, e.id);
        }
    }
}
...
inline void SupervisorCtl::broadcast_events_health(const HealthBeacon& msg) {
    std::vector<EventsHealthEntry> snap;
    {
        std::lock_guard<std::mutex> lk(events_health_mu_);
        snap = events_health_subs_;
    }
    for (const auto& e : snap) {
        try { e.fn(msg); }
        catch (...) {
            std::fprintf(stderr,
                "[%s] subscriber %u threw on events.health — dropping\n",
                kNodeName, e.id);
        }
    }
}
...
inline void SupervisorCtl::broadcast_events_snap_begin(const SnapshotBegin& msg) {
    std::vector<EventsSnap_beginEntry> snap;
    {
        std::lock_guard<std::mutex> lk(events_snap_begin_mu_);
        snap = events_snap_begin_subs_;
    }
    for (const auto& e : snap) {
        try { e.fn(msg); }
        catch (...) {
            std::fprintf(stderr,
                "[%s] subscriber %u threw on events.snap_begin — dropping\n",
                kNodeName, e.id);
        }
    }
}
...
inline void SupervisorCtl::broadcast_events_edge(const NodeEdge& msg) {
    std::vector<EventsEdgeEntry> snap;
    {
        std::lock_guard<std::mutex> lk(events_edge_mu_);
        snap = events_edge_subs_;
    }
    for (const auto& e : snap) {
        try { e.fn(msg); }
        catch (...) {
            std::fprintf(stderr,
                "[%s] subscriber %u threw on events.edge — dropping\n",
                kNodeName, e.id);
        }
    }
}
...
inline void SupervisorCtl::broadcast_events_node_state(const NodeState& msg) {
    std::vector<EventsNode_stateEntry> snap;
    {
        std::lock_guard<std::mutex> lk(events_node_state_mu_);
        snap = events_node_state_subs_;
    }
    for (const auto& e : snap) {
        try { e.fn(msg); }
        catch (...) {
            std::fprintf(stderr,
                "[%s] subscriber %u threw on events.node_state — dropping\n",
                kNodeName, e.id);
        }
    }
}
...
inline void SupervisorCtl::broadcast_events_snap_end(const SnapshotEnd& msg) {
    std::vector<EventsSnap_endEntry> snap;
    {
        std::lock_guard<std::mutex> lk(events_snap_end_mu_);
        snap = events_snap_end_subs_;
    }
    for (const auto& e : snap) {
        try { e.fn(msg); }
        catch (...) {
