...
// The aggregate-tree prefix is the machine NAME — the unique RUNTIME identity com
// resolves from the manifest's machine_index (instance N → machine_index → name).
// serialize-manifest enforces unique names + a sequential index, so each board's
// name is distinct and the merged forest keeps every machine as its own subtree.
...
bool machine_name_to_instance(const std::string& sel, uint32_t& inst) {
    ...
    // An all-digits selector is the INSTANCE number (master=0, compute=1, …) — a
    // convenience matching the INST column `rtdb machines` prints.
