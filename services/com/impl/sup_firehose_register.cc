...
#include "lib/supervisor_codecs.hh"
...
void register_firehose_casts(theia::runtime::NodeBinding* b) {
    ...
    install<system_supervisor_SnapshotBegin>(
        b, [&fh](const system_supervisor_SnapshotBegin& m) {
            fh.on_snapshot_begin(m.generation, m.timestamp_ms);
        });
    install<system_supervisor_NodeEdge>(
        b, [&fh](const system_supervisor_NodeEdge& m) {
            fh.on_node_edge(m.op, m.parent_name, m.name, m.kind);
        });
    install<system_supervisor_NodeState>(
        b, [&fh](const system_supervisor_NodeState& m) {
    ...
    install<system_supervisor_SnapshotEnd>(
        b, [&fh](const system_supervisor_SnapshotEnd& m) {
