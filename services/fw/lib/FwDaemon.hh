...
inline void FwDaemon::broadcast_broadcast_status(const FwStatusMsg& msg) {
    ...
    // `msg` to EACH watched member. group identity = msg_type_name<FwStatusMsg>().
    // The impl can call pg_members<FwStatusMsg>() first to skip work when the list
    // is empty (e.g. logcat stop-tailing) — this default just fans out to all.
    pg_watch<FwStatusMsg>();
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
