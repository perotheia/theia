...
inline void IdsmDaemon::broadcast_broadcast_status(const IdsStatusMsg& msg) {
    ...
    // `msg` to EACH watched member. group identity = msg_type_name<IdsStatusMsg>().
    // The impl can call pg_members<IdsStatusMsg>() first to skip work when the list
    // is empty (e.g. logcat stop-tailing) — this default just fans out to all.
    pg_watch<IdsStatusMsg>();
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
    if (!pb_encode(&_os, ::theia::runtime::RemoteCodec<IdsStatusMsg>::fields(), &msg))
        return;
    pg_.broadcast_members<IdsStatusMsg>(_buf, static_cast<uint16_t>(_os.bytes_written));
}

inline void IdsmDaemon::broadcast_to_sm_mode_input(const IdsmModeInput& msg) {
    // Ensure we're monitoring the group (OTP pg:monitor — idempotent), then cast
    // `msg` to EACH watched member. group identity = msg_type_name<IdsmModeInput>().
    // The impl can call pg_members<IdsmModeInput>() first to skip work when the list
    // is empty (e.g. logcat stop-tailing) — this default just fans out to all.
    pg_watch<IdsmModeInput>();
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
