...
int main(int argc, char** argv) {
    ...
    idsm_daemon.set_tipc_instance(idsm_daemon_inst);
    ...
    const bool idsm_daemon_enabled = idsm_daemon_params.boolean("enabled", true);
    if (!idsm_daemon_enabled) {
        idsm_daemon.log().info("node disabled (params.enabled=false) — not "
    ...
    }  // end if (idsm_daemon_enabled) — PASS 1 (mux wiring)
    ...
    if (idsm_daemon_params.boolean("enabled", true)) {
    if (auto idsm_daemon_delay = idsm_daemon_params.u32("start_delay_ms", 0))
        std::this_thread::sleep_for(std::chrono::milliseconds(idsm_daemon_delay));
    idsm_daemon.start();
    {
        char _tipc[64];
    ...
    }  // end if (idsm_daemon_enabled) — PASS 2 (start + heartbeat)
