...
inline void NmDaemon::broadcast_broadcast_status(const NmStatusMsg& msg) {
    // group identity = msg_type_name<NmStatusMsg>() (the .art wire name); the
    // supervisor allocates group_type on first resolve. Members pg_join the same
    // type → register_cast<NmStatusMsg> delivers it to their handle_cast.
    auto _g = pg_resolve<NmStatusMsg>();
    if (!_g.ok) return;                       // supervisor unreachable — drop
    pg_broadcast<NmStatusMsg>(_g.type, msg);
}
