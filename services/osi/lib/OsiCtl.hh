...
#include "osi_codecs.hh"   // FC-wide inbound RemoteCodecs (#387)
...
// ara::osi`) so it isn't nested under this file's
// namespace block — bound as the GenServer State type below.
#include "impl/OsiCtl_state.hh"
...
// (system_services_osi_*); aliasing them here keeps callers'
// signatures readable.
using ResourceStatus = system_services_osi_ResourceStatus;
using ResourceStatusReq = system_services_osi_ResourceStatusReq;
using ResourceLimitReq = system_services_osi_ResourceLimitReq;
using ResourceLimitReply = system_services_osi_ResourceLimitReply;
...
class OsiCtl : public ::theia::runtime::GenServer<OsiCtl, OsiCtlState> {
public:
    static constexpr const char* kNodeName = "osi_ctl";
    static constexpr uint32_t kTipcType     = 0x8001000Cu;
    ...
    using ::theia::runtime::GenServer<OsiCtl, OsiCtlState>::handle_cast;
    ...
    // (post_info / self-tick).
    void terminate(const char* /*reason*/, OsiCtlState& /*s*/) noexcept {}
    ...
    ResourceStatus handle_call(const ResourceStatusReq& req,
                                            OsiCtlState& s);

    ResourceLimitReply handle_call(const ResourceLimitReq& req,
                                            OsiCtlState& s);


    // ---- OTP init/1 — body in impl (OsiCtl_handlers.cc) ----------
    //
    ...
    void handle_info(const char* info, OsiCtlState& s);

    // ---- config update — body in impl (OsiCtl_handlers.cc) -------
    //
    // This node binds `config OsiConfig`. services/per casts a
    ...
    // ParseFromString cfg.config into OsiConfig and honor the changed mask.
    ...
    void broadcast_broadcast_status(const ResourceStatus& msg);
...
inline void OsiCtl::broadcast_broadcast_status(const ResourceStatus& msg) {
    // Ensure we're monitoring the group (OTP pg:monitor — idempotent), then cast
    // `msg` to EACH watched member. group identity = msg_type_name<ResourceStatus>().
    // The impl can call pg_members<ResourceStatus>() first to skip work when the list
    // is empty (e.g. logcat stop-tailing) — this default just fans out to all.
    pg_watch<ResourceStatus>();
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
    if (!pb_encode(&_os, ::theia::runtime::RemoteCodec<ResourceStatus>::fields(), &msg))
        return;
    pg_.broadcast_members<ResourceStatus>(_buf, static_cast<uint16_t>(_os.bytes_written));
}

inline void OsiCtl::broadcast_to_phm_report(const FcHealthReport& msg) {
    // Ensure we're monitoring the group (OTP pg:monitor — idempotent), then cast
