...
inline void OsiCtl::broadcast_broadcast_status(const ResourceStatus& msg) {
    ...
    // bound member (no per-member loop). group identity = msg_type_name<ResourceStatus>()
    // (the .art wire name); the supervisor allocates group_type the first time we
    // resolve it. Members receive it via register_cast<ResourceStatus> (same service_id),
    // so the receive path is the node's normal handle_cast. Best-effort (lossy).
    auto _g = pg_resolve<ResourceStatus>();
    if (!_g.ok) return;                       // supervisor unreachable — drop
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
    if (!pb_encode(&_os, ::theia::runtime::RemoteCodec<ResourceStatus>::fields(), &msg))
        return;
    pg_.broadcast<ResourceStatus>(_g.type, _buf, static_cast<uint16_t>(_os.bytes_written));
}
