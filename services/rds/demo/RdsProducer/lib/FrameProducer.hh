...
// impl/FrameProducer_handlers.cc.

#pragma once

#include "GenServer.hh"
#include "Log.hh"      // sibling — per-FC ContextLogger + kLogTag
#include "rdsdemo_codecs.hh"   // FC-wide inbound RemoteCodecs (#387)
#include "system/services/rdsdemo/rdsdemo.pb.h"
...
// ara::rdsdemo`) so it isn't nested under this file's
// namespace block — bound as the GenServer State type below.
#include "impl/FrameProducer_state.hh"
...
// (system_services_rdsdemo_*); aliasing them here keeps callers'
// signatures readable.
...
class FrameProducer : public ::theia::runtime::GenServer<FrameProducer, FrameProducerState> {
public:
    static constexpr const char* kNodeName = "frame_producer";
    static constexpr uint32_t kTipcType     = 0x80010040u;
    ...
    using ::theia::runtime::GenServer<FrameProducer, FrameProducerState>::handle_cast;
    ...
    // (post_info / self-tick).
    void terminate(const char* /*reason*/, FrameProducerState& /*s*/) noexcept {}
    ...
    // ---- OTP init/1 — body in impl (FrameProducer_handlers.cc) ----------
    //
    ...
    void init(FrameProducerState& s);
    ...
    void handle_info(const char* info, FrameProducerState& s);
