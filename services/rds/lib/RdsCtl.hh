...
#include "rds_codecs.hh"   // FC-wide inbound RemoteCodecs (#387)
#include "system/services/rds/rds.pb.h"
...
// ara::rds`) so it isn't nested under this file's
// namespace block — bound as the GenServer State type below.
#include "impl/RdsCtl_state.hh"
...
// (system_services_rds_*); aliasing them here keeps callers'
// signatures readable.
using GetRdsStatus = system_services_rds_GetRdsStatus;
using RdsStatus = system_services_rds_RdsStatus;
...
class RdsCtl : public ::theia::runtime::GenServer<RdsCtl, RdsCtlState> {
public:
    static constexpr const char* kNodeName = "ctl";
    static constexpr uint32_t kTipcType     = 0x80010098u;
    ...
    using ::theia::runtime::GenServer<RdsCtl, RdsCtlState>::handle_cast;
    ...
    // (post_info / self-tick).
    void terminate(const char* /*reason*/, RdsCtlState& /*s*/) noexcept {}
    ...
    RdsStatus handle_call(const GetRdsStatus& req,
                                            RdsCtlState& s);


    // ---- OTP init/1 — body in impl (RdsCtl_handlers.cc) ----------
    //
    ...
    void handle_info(const char* info, RdsCtlState& s);
