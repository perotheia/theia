...
int main(int argc, char** argv) {
    ...
    if (const char* td = std::getenv("THEIA_TOMBSTONE_DIR")) {
        if (td[0]) tombstone::install_handlers("rds", td);
    }
    ...
    // Path from $THEIA_CONFIG / $THEIA_CONFIG_DIR / ./config/rds.json.
    ::theia::runtime::init_config("rds");
    ...
    const bool roudi_enabled = roudi_params.boolean("enabled", true);
    if (!roudi_enabled) {
        roudi.log().info("node disabled (params.enabled=false) — not "
    ...
    ctl.set_tipc_instance(ctl_inst);
    // Generic node params (read via the static-deploy ParamsConfig, overridable
    // per-rig in config/<fc>.json under this node's section — same mechanism as
    // tsync's prebuilt `enabled`):
    //   enabled          (default true)  — boot gate; a disabled node does NOT
    ...
    // start() ran here (before config_mux.start()), a pg_join in init() would
    // deadlock, the node would never beat, and the watchdog would SIGTERM it.
    const bool ctl_enabled = ctl_params.boolean("enabled", true);
    if (!ctl_enabled) {
        ctl.log().info("node disabled (params.enabled=false) — not "
            "started; its TIPC address is free for a test/probe to bind");
    } else {
    ...
    if (auto* ctl_cfg = config_mux.bind_node(
        ...
        config_mux.register_call<GetRdsStatus, RdsStatus>(
            ctl_cfg, ctl);
    ...
    }  // end if (ctl_enabled) — PASS 1 (mux wiring)
    ...
    if (roudi_params.boolean("enabled", true)) {
    if (auto roudi_delay = roudi_params.u32("start_delay_ms", 0))
        std::this_thread::sleep_for(std::chrono::milliseconds(roudi_delay));
    roudi.start();
    {
        char _tipc[64];
    ...
    // PASS 2 — start the node thread (runs init()/1) now that the mux pumps,
    // then apply post-start settings (affinity) + the liveness heartbeat.
    if (ctl_params.boolean("enabled", true)) {
    if (auto ctl_delay = ctl_params.u32("start_delay_ms", 0))
        std::this_thread::sleep_for(std::chrono::milliseconds(ctl_delay));
    ctl.start();
    {
        char _tipc[64];
        std::snprintf(_tipc, sizeof(_tipc), "up — TIPC type=0x%x instance=%u",
    ...
    // node, own timer thread; 1s default matches the supervisor's check cadence.
    {
        ...
        if (ctl_hb->open()) {
            ...
            ctl.log().warn("heartbeat publisher open failed; "
                                     "supervisor watchdog will not see beats");
        }
    }

    }  // end if (ctl_enabled) — PASS 2 (start + heartbeat)
