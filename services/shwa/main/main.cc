...
int main(int argc, char** argv) {
    ...
    shwa_daemon.set_tipc_instance(shwa_daemon_inst);
    ...
    const bool shwa_daemon_enabled = shwa_daemon_params.boolean("enabled", true);
    if (!shwa_daemon_enabled) {
        shwa_daemon.log().info("node disabled (params.enabled=false) — not "
    ...
    }  // end if (shwa_daemon_enabled) — PASS 1 (mux wiring)
    ...
    if (shwa_daemon_params.boolean("enabled", true)) {
    if (auto shwa_daemon_delay = shwa_daemon_params.u32("start_delay_ms", 0))
        std::this_thread::sleep_for(std::chrono::milliseconds(shwa_daemon_delay));
    shwa_daemon.start();
    {
        char _tipc[64];
    ...
    }  // end if (shwa_daemon_enabled) — PASS 2 (start + heartbeat)
