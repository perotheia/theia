...
inline void SmDaemon::broadcast_broadcast_state(const SmStateMsg& msg) {
    // group identity = msg_type_name<SmStateMsg>() (the .art wire name); the
    // supervisor allocates group_type on first resolve. Members pg_join the same
    // type → register_cast<SmStateMsg> delivers it to their handle_cast.
    auto _g = pg_resolve<SmStateMsg>();
    if (!_g.ok) return;                       // supervisor unreachable — drop
    pg_broadcast<SmStateMsg>(_g.type, msg);
}
