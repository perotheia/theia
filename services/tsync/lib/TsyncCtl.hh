...
inline void TsyncCtl::broadcast_navsatfix_fix(const NavSatFix& msg) {
    ...
    // bound member (no per-member loop). group identity = msg_type_name<NavSatFix>()
    // (the .art wire name); the supervisor allocates group_type the first time we
    // resolve it. Members receive it via register_cast<NavSatFix> (same service_id),
    // so the receive path is the node's normal handle_cast. Best-effort (lossy).
    auto _g = pg_resolve<NavSatFix>();
    if (!_g.ok) return;                       // supervisor unreachable — drop
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
    if (!pb_encode(&_os, ::theia::runtime::RemoteCodec<NavSatFix>::fields(), &msg))
        return;
    pg_.broadcast<NavSatFix>(_g.type, _buf, static_cast<uint16_t>(_os.bytes_written));
}

inline void TsyncCtl::broadcast_gnss_odom_odom(const Odometry& msg) {
    // OTP-pg multicast: encode `msg` once and send ONE datagram to its TYPE's
    // process group {group_type, 0..~0} — the TIPC kernel fans out a copy to every
    // bound member (no per-member loop). group identity = msg_type_name<Odometry>()
    // (the .art wire name); the supervisor allocates group_type the first time we
    // resolve it. Members receive it via register_cast<Odometry> (same service_id),
    // so the receive path is the node's normal handle_cast. Best-effort (lossy).
    auto _g = pg_resolve<Odometry>();
    if (!_g.ok) return;                       // supervisor unreachable — drop
    uint8_t _buf[8192];
    pb_ostream_t _os = pb_ostream_from_buffer(_buf, sizeof(_buf));
    if (!pb_encode(&_os, ::theia::runtime::RemoteCodec<Odometry>::fields(), &msg))
        return;
    pg_.broadcast<Odometry>(_g.type, _buf, static_cast<uint16_t>(_os.bytes_written));
}
