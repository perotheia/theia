...
inline void UcmFsm::broadcast_progress_progress(const UcmProgress& msg) {
    // group identity = msg_type_name<UcmProgress>() (the .art wire name); the
    // supervisor allocates group_type on first resolve. Members pg_join the same
    // type → register_cast<UcmProgress> delivers it to their handle_cast.
    auto _g = pg_resolve<UcmProgress>();
    if (!_g.ok) return;                       // supervisor unreachable — drop
    pg_broadcast<UcmProgress>(_g.type, msg);
}
