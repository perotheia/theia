...
// ara::vucm`) so it isn't nested under this file's
...
// signatures readable.
using CampaignRequest = system_services_vucm_CampaignRequest;
...
// derives from VucmGateStateMBase (gen_statem-shaped); without
...
class VucmGate : public ::theia::runtime::GenServer<VucmGate, VucmGateState> {
public:
    static constexpr const char* kNodeName = "vucm_gate";
    static constexpr uint32_t kTipcType     = 0x8001005Eu;
    ...
    using ::theia::runtime::GenServer<VucmGate, VucmGateState>::handle_cast;
    ...
    // (post_info / self-tick).
    void terminate(const char* /*reason*/, VucmGateState& /*s*/) noexcept {}
    ...
    void handle_cast(const EvDeployment& msg, VucmGateState& s);

    void handle_cast(const EvPlanned& msg, VucmGateState& s);
    ...
    CampaignReply handle_call(const CampaignRequest& req,
                                            VucmGateState& s);
    ...
    // ---- OTP init/1 — body in impl (VucmGate_handlers.cc) ----------
    //
    ...
    void handle_info(const char* info, VucmGateState& s);
