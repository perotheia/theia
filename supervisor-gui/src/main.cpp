...
//
// Standard wxApp boilerplate. Loads the per-rig machines.yaml (output
// of `artheia gui emit <rig>`) and hands the endpoint list to MainFrame.
// Without --machines-yaml, falls back to a localhost-only list so the
// single-supervisor demo still runs unconfigured.
...
public:
    bool OnInit() override {
        if (!wxApp::OnInit()) return false;  // runs cmdline parser
        ...
        if (!machines_yaml_.empty()) {
            ...
            if (machines.empty()) {
                std::fprintf(stderr,
                    ...
                    "falling back to localhost:7610\n",
