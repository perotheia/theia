...
// operator can see crashes and restart cascades as they happen. Multi-
// machine: each row carries the machine_name it arrived on.
//
// Tombstone events surface here too (kind=3); OTP observer doesn't
// have a dedicated tombstones tab, and a single trace stream matches
// the source-of-truth shape.
...
TracePanel::TracePanel(wxWindow* parent) : PanelBase(parent) {
    ...
    list_->InsertColumn(1, "machine",     wxLIST_FORMAT_LEFT,  120);
    list_->InsertColumn(2, "kind",        wxLIST_FORMAT_LEFT,  140);
    list_->InsertColumn(3, "child",       wxLIST_FORMAT_LEFT,  180);
    list_->InsertColumn(4, "supervisor",  wxLIST_FORMAT_LEFT,  140);
    list_->InsertColumn(5, "pid",         wxLIST_FORMAT_RIGHT,  80);
    list_->InsertColumn(6, "code",        wxLIST_FORMAT_RIGHT,  60);
    list_->InsertColumn(7, "detail",      wxLIST_FORMAT_LEFT,  400);
...
void TracePanel::on_frame(const std::string& machine_name,
    ...
    list_->SetItem(row, 1, machine_name);
    list_->SetItem(row, 2, kind_name(ev.kind()));
    list_->SetItem(row, 3, ev.child_name());
    list_->SetItem(row, 4, ev.supervisor_name());
    list_->SetItem(row, 5, wxString::Format("%d", ev.pid()));
    list_->SetItem(row, 6, wxString::Format("%d", ev.exit_code()));
