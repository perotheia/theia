"""Hermetic Robot library for the gen-fc application-generation
selftest.
...
Pipeline stages exercised:
  ...
  1. Initial gen-fc emit (lib + main + impl + proto)
...
class GenAppChainLib:
    ...
    def wipe_synthetic_fc(self) -> None:
        """Remove every file gen-fc could have emitted for the duo FC.
    ...
    def _run_gen_app(self, *extra_args: str) -> str:
        ...
        if result.returncode != 0:
            raise AssertionError(
                f"gen-fc failed (rc={result.returncode}):\n"
    ...
    def add_signal_to_art(self, signal_name: str, message_name: str) -> None:
        ...
        (the daemon). PongerNode is what gen-fc's lib template wires
        broadcasters/subscribers for, so the new interface MUST show up
        in lib/PingerNode.hh after re-emit.

        Wired on PongerNode (not PingerNode) deliberately — gen-fc
    ...
    def _emit_proto_build(self) -> None:
        """gen-fc emits the .proto under platform/proto/system/services/duo/
        ...
        if not src_proto.exists():
            raise AssertionError(
                f"gen-fc didn't write expected proto at {src_proto}"
            )
